# Sell Review — 2026-06-10T20:20:00Z

**0 sells proposed. All 12 positions HOLD.**

Tape context (Jun 9 close): SPY −1.36%, VIX 21.12 — risk_off. Technology −2.33%, Energy −2.96%.  
Holdings snapshot: 2026-06-09T21:00:25Z. Candidates: 2026-06-09T16:07:51Z.

Web search this run: **PMN** — ProMIS Neurosciences PRECISE-AD Phase 1b fully enrolled; blinded 6-month interim data expected early Q3 2026; Oppenheimer CNS Summit presentation June 10, 2026 (today). No clinical holds, no safety signals, no thesis-breaking events found. The −8.24% is risk-off/biotech-sector weakness; the forward catalyst is intact.

---

## Per-Position Rulings

**APLD — HOLD**  
Opened Jun 9, +1.40% ($41.44 → $42.02). Applied Digital AI/HPC data center. 8-K filed Jun 9: $1.59B senior secured notes + $550M revolver (Goldman) + CoreWeave lease-assignment MOU. Fresh financing confirms the data center buildout thesis is live. Green on a risk_off tape. Thesis intact.

**ATHE — HOLD**  
Opened Jun 9, +2.36% ($4.23 → $4.33). Alterity Therapeutics. FDA accepted Phase 3 plan for MSA drug that slowed neurological decline 48% in Phase 2 (Bullish 0.42 in Jun 9 candidates). The FDA Phase 3 acceptance is the *start* of a durable multi-month clinical thesis — Phase 3 enrollment has not yet begun and the data readout is many months away. Catalyst not spent; it just launched. HOLD.

**AVR — HOLD**  
Opened Jun 8, −4.05% ($9.87 → $9.47). Anteris Technologies. Thesis: PARADIGM global pivotal trial. Not in Jun 9 candidates funnel; no AVR-specific negative event identified. The −4.05% is risk_off price noise (Healthcare sector −0.34% Jun 9) with no company-specific catalyst change. Thesis intact. HOLD.

**EVTL — HOLD**  
Opened Jun 9, −0.94% ($2.12 → $2.10). Vertical Aerospace. Final full-scale Valo prototype completed first piloted flight ahead of Critical Design Review (CDR) — in Jun 9 candidates (Bullish 0.487). CDR and subsequent production gate still ahead. Trivial drawdown on a broadly red tape. Thesis in progress. HOLD.

**GASS — HOLD**  
Opened Jun 5, −1.26% ($9.51 → $9.39). StealthGas LPG shipping. In Jun 9 candidates: Seeking Alpha states "The Rally Has Not Fully Closed The Value Gap" (Neutral 0.04) — independent confirmation the thesis is unspent. Energy sector −2.96% Jun 9 is broad macro drag, not a GASS-specific event. HOLD.

**GIII — HOLD**  
Opened Jun 8, −2.30% ($34.80 → $34.00). G-III Apparel. Not in Jun 9 candidates; no earnings revision, guidance cut, or tariff/supply-chain event identified. Consumer Cyclical −1.05% Jun 9 is sector-level risk_off drag only. Thesis intact. HOLD.

**NRXP — HOLD**  
Opened Jun 5, −6.16% ($4.22 → $3.96). NRx Pharmaceuticals. Thesis: IV ketamine ANDA (KETAFREE™), FDA GDUFA goal date July 29, 2026. Not in Jun 9 candidates; no FDA action, clinical hold, or competitive event identified. The −6.16% is a drawdown on a position that was −10.4% mid-session Jun 9 and recovered — price is moving toward thesis, not away. HOLD.

**PLSE — HOLD**  
Opened Jun 5, +1.10% ($25.49 → $25.77). Pulse Biosciences. Position is green. Not in Jun 9 candidates; no negative analyst action or clinical setback identified. Thesis intact. HOLD.

**PMN — HOLD**  
Opened Jun 9, −8.24% ($10.62 → $9.745). ProMIS Neurosciences. **Web searched this run.** PRECISE-AD Phase 1b fully enrolled; blinded 6-month interim data expected early Q3 2026; Oppenheimer CNS Summit presentation Jun 10 (today). No clinical holds, no safety signals, no thesis-breaking news found. The −8.24% move was on a risk_off day (SPY −1.36%, biotech broadly weak). Forward catalyst (Q3 interim data) intact. HOLD.

**RXRX — HOLD**  
Opened Jun 5, −1.82% ($3.29 → $3.23). Recursion Pharmaceuticals. Technology sector fell −2.33% Jun 9; RXRX at −1.82% is outperforming its sector. Not in Jun 9 candidates; no pipeline failure or partnership dissolution identified. Thesis intact. HOLD.

**SLDB — HOLD**  
Opened Jun 4, −2.33% ($6.86 → $6.70). Solid Biosciences gene therapy. In Jun 9 candidates (Bullish 0.442): "Gene therapy programs show robust efficacy, safety, and regulatory momentum." Active positive catalyst coverage in today's funnel — thesis explicitly confirmed live. HOLD.

**TISI — HOLD**  
Opened Jun 4, −3.26% ($18.09 → $17.50). Team Inc. industrial services. Not in Jun 9 candidates; no contract cancellation, earnings miss, or negative guidance identified. Industrials fell −0.91% Jun 9 — decline is sector-correlated. Thesis intact. HOLD.

---

## Summary Table

| Symbol | Opened | Unrlzd%  | Key Thesis Status                                          | Decision |
|--------|--------|----------|------------------------------------------------------------|----------|
| APLD   | Jun 9  | +1.40%   | 8-K financing + CoreWeave MOU live (AI data center)        | HOLD |
| ATHE   | Jun 9  | +2.36%   | FDA Phase 3 acceptance — thesis just launched              | HOLD |
| AVR    | Jun 8  | −4.05%   | PARADIGM pivotal trial ongoing; no adverse event           | HOLD |
| EVTL   | Jun 9  | −0.94%   | CDR upcoming; prototype flight milestone logged            | HOLD |
| GASS   | Jun 5  | −1.26%   | SA confirms value gap still open; energy sector drag       | HOLD |
| GIII   | Jun 8  | −2.30%   | Sector drag only; no company-specific break                | HOLD |
| NRXP   | Jun 5  | −6.16%   | GDUFA Jul 29 intact; price recovered from session low      | HOLD |
| PLSE   | Jun 5  | +1.10%   | Green; no adverse events                                   | HOLD |
| PMN    | Jun 9  | −8.24%   | Q3 interim data intact; conference today; risk-off only    | HOLD |
| RXRX   | Jun 5  | −1.82%   | Outperforming Tech sector; no pipeline miss                | HOLD |
| SLDB   | Jun 4  | −2.33%   | Active Bullish catalyst in funnel; regulatory momentum     | HOLD |
| TISI   | Jun 4  | −3.26%   | Industrials sector drag; no adverse event                  | HOLD |
