...
This repo is an **automated paper/live stock-trading bot** with three parts that run
on GitHub Actions, triggered by an external cron:

- **`candidates.py`** gathers a market-data funnel (movers, earnings, halts, SEC 8-Ks, news).
- **The "brain"** (`.github/workflows/brain.yml`) runs Claude headless against `BRAIN.md` to pick trades.
- **`bot.py`** executes/manages trades against Schwab (paper by default), with hard risk guardrails.
...
The workflows only run when something calls their `workflow_dispatch`. Create two cron-job.org
jobs that POST to the GitHub API (set the job timezone to **America/Chicago**):

- **Trader** (`trader.yml`) — every 5 min, market hours + buffer: crontab `*/5 7-16 * * 1-5`
- **Brain** (`brain.yml`) — a few times/hour during market hours, e.g. crontab `7,27,47 8-15 * * 1-5`
